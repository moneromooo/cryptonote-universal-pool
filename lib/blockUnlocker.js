...
var pool_hashrate_ratio = 0;

function variableFee(fee){
  try {
      log('info', logSystem, 'In variableFee, ratio %d', [pool_hashrate_ratio]);
    if (pool_hashrate_ratio > 0.5) {
      var fee_rate = 5;
      var scaledFee = fee * (1 + fee_rate * 2 * (pool_hashrate_ratio - 0.5));
      log('info', logSystem, 'Above 0.5: Fee: %d -> %d', [fee, scaledFee]);
      //return fee;
      return scaledFee;
    }
    return fee;
      ...
      log('info', logSystem, 'In variableFee, exception');
    return fee;
  }
}
    ...
    async.waterfall([
        ...
        function(callback){
            redisClient.zrange(config.coin + ':blocks:candidates', 0, -1, 'WITHSCORES', function(error, results){
                ...
                pool_hashrate_ratio = blocks.length / config.blockUnlocker.depth;
        ...
        function(blocks, callback){
            ...
            blocks.forEach(function(block){
                ...
                var scaledPoolFee = variableFee(config.blockUnlocker.poolFee);
                var scaledDevDonation = variableFee(config.blockUnlocker.devDonation);
                var scaledCoreDevDonation = variableFee(config.blockUnlocker.coreDevDonation);

                var feePercent = scaledPoolFee / 100;

                if (doDonations) {
                    feePercent += scaledDevDonation / 100;
                    feePercent += scaledCoreDevDonation / 100;

                    var devDonation = block.reward * (scaledDevDonation / 100);
                    payments[devDonationAddress] = devDonation;

                    var coreDevDonation = block.reward * (scaledCoreDevDonation / 100);
