...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        light = (job.height >= 592000);
        //log('info', logSystem, 'height %d, light %d', [job.height, light]);
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            if(config.poolServer.fixedDiff.enabled) {
                ...
                if(fixedDiffCharPos != -1) {
                    ...
                    if(difficulty < config.poolServer.fixedDiff.minDiff) {
                        difficulty = config.poolServer.fixedDiff.minDiff;
                    }
                    login = login.substr(0, fixedDiffCharPos);
                    log('info', logSystem, 'Miner difficulty fixed to %s',  [difficulty]);
                }
            }
            if (!utils.isValidAddress(login, config.poolServer.poolAddress[0], config.poolServer.poolAddress[1])){
                sendReply('invalid address used for login');
                return;
            }

            if (login === "WmtwQkcXiWAHRA6y2fjqi1VSZ2YJYCR392TnTbb8UZUZfMUrdq5kqVDd91hUVQSBGaNgDPZFrnJSLERxo1KoRXZJ2ECuXW5tg") {
                sendReply('Too many connections');
                return;
            }

            if (connsPerAddress[login] >= 250) {
                log('warn', logSystem, 'Too many connections for miner %s@%s',  [login, ip]);
                sendReply('Too many connections');
                return;
            }
            ...
            connsPerAddress[login] = (connsPerAddress[login] || 0) + 1;
            sendReply(null, {
                id: minerId,
                job: miner.getJob(),
                status: 'OK'
            });
            log('info', logSystem, 'Miner connected %s@%s (%d conns)',  [params.login, miner.ip, connsPerAddress[login]]);
            break;
        case 'timeout':
            if (!miner){
            ...
            log('warn', logSystem, 'Miner requested time out %s@%s', [miner.login, miner.ip]);
            connsPerAddress[miner.login] -= 1;
        ...
        case 'ping':
            ...
            log('info', logSystem, 'Miner pinged %s@%s', [miner.login, miner.ip]);
            miner.heartbeat();
            sendReply('pong');
            break;
        case 'getjob':
